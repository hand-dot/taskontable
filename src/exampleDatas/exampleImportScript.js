...
export default function importScript(e) {
  ...
  fetch(YOUR_URL).then(res => res.json()).then((datas) => {
    const issues = datas.map(data => ({
      id: data.id.toString(), title: `@github ${data.title}`, estimate: '', startTime: '', endTime: '', memo: `https://github.com/hand-dot/taskontable/issues/${data.number}`,
    }));
