...
function TableCtl(props) {
  const { tableTasks, hasOpenTask, date, lastSaveTime, saveable, changeDate, saveHot, classes, theme } = props;
  return (
          ...
          if (theme.breakpoints.values.sm < constants.APPWIDTH) {
            return (
              <Tooltip title={`最終保存時刻 : ${(`00${lastSaveTime.hour}`).slice(-2)}:${(`00${lastSaveTime.minute}`).slice(-2)}`} placement="top">
                <div style={{ display: 'inline-block' }}>
                  <Button className={classes.tableCtlButton} disabled={!saveable} variant="raised" onClick={saveHot} color="default"><i className="fa fa-floppy-o fa-lg" /></Button>
                </div>
              </Tooltip>
            );
          }
